import Apps from "gi://AstalApps";
import { register } from "ags/gobject";
import { BaseProvider } from "../SearchProvider";
import { AppItem, ProviderConfig } from "../types";
import GLib from "gi://GLib";

@register({ GTypeName: "AppProvider" })
export class AppProvider extends BaseProvider {
  readonly config: ProviderConfig = {
    command: "app",
    icon: "Apps",
    name: "Apps",
    placeholder: "Search apps...",
    component: "list",
    maxResults: 8,
  };

  private apps = new Apps.Apps();
  private allApps: AppItem[] = [];
  private pinnedAppIds: string[] = [];

  constructor() {
    super();
    this.command = "app";
    this.loadPinnedApps();
    this.loadAllApps();
  }

  private loadPinnedApps(): void {
    try {
      const configPath = `${GLib.get_home_dir()}/.config/ags/configs/pickerapps.json`;
      const [success, contents] = GLib.file_get_contents(configPath);
      
      if (success) {
        const decoder = new TextDecoder();
        const jsonString = decoder.decode(contents);
        const config = JSON.parse(jsonString);
        this.pinnedAppIds = config.pinned || [];
      }
    } catch (error) {
      console.warn("Could not load pinned apps config:", error);
      this.pinnedAppIds = [];
    }
  }

  private loadAllApps(): void {
    const rawApps = this.apps.get_list() || [];
    // Add a frecency caching ID
    this.allApps = rawApps.map((app) => {
      app.id = this.generateAppId(app);
      return app;
    });
  }

  private generateAppId(app: any): string {
    if (app.executable) {
      const execParts = app.executable.split(" ")[0].split("/");
      const execName = execParts[execParts.length - 1];
      if (execName && execName.length > 1) {
        return execName.toLowerCase();
      }
    }
    return "unknown-app";
  }

  private getPinnedApps(): AppItem[] {
    const pinnedApps: AppItem[] = [];
    
    for (const desktopId of this.pinnedAppIds) {
      const app = this.allApps.find(a => {
        // Check if the app's entry matches the desktop ID
        return a.entry && a.entry.endsWith(desktopId);
      });
      
      if (app) {
        pinnedApps.push(app);
      }
    }
    
    return pinnedApps;
  }

  async search(query: string): Promise<void> {
    this.setLoading(true);
    try {
      const trimmedQuery = query.trim();
      if (trimmedQuery.length === 0) {
        // Show pinned apps when query is empty
        const pinnedApps = this.getPinnedApps();
        if (pinnedApps.length > 0) {
          this.setResults(pinnedApps);
        } else {
          // Fallback to all apps if no pinned apps found
          this.setResults(this.allApps.slice(0, this.config.maxResults));
        }
      } else {
        const fuzzyResults = this.apps
          .fuzzy_query(trimmedQuery)
          .slice(0, this.config.maxResults);
        this.setResults(fuzzyResults);
      }
    } finally {
      this.setLoading(false);
    }
  }

  activate(item: AppItem): void {
    item.launch();
  }

  async refresh(): Promise<void> {
    this.loadPinnedApps();
    this.loadAllApps();
    if (this.showingDefaults) {
      const pinnedApps = this.getPinnedApps();
      if (pinnedApps.length > 0) {
        this.setResults(pinnedApps);
      } else {
        this.setResults(this.allApps.slice(0, this.config.maxResults));
      }
    }
  }
}